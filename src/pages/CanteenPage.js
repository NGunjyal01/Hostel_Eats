...
const CanteenPage = () => {
  ...
  const cartItemMap = !Object.keys(cart).length
    ? new Map()
  ...
  const handleSearchSubmit = async (e) => {
    e.preventDefault();
    if (!searchInput.trim()) {
        toast.error('Please enter the name of a dish.');
        return;
      }
    const formData = {
      shopid: canteenId,
  ...
  const handleAdd = async (e, itemid, shopid) => {
    e.stopPropagation();
    const cartCanteenId = !Object.keys(cart).length
      ? null
      : cart.items[0].item.shopid._id;
  ...
  return (
    <div className="bg-gradient-to-r from-black to-[#222831] min-h-screen p-6 text-white pt-24">
      {loading ? (
        <div className="mt-[65%] sm:mt-[20%] ml-[40%] sm:ml-[47%]">
          <Spinner />
        </div>
      ) : (
        <>
          <div className="bg-[#31363F] w-full sm:w-7/12 mx-auto p-4 sm:p-6 rounded-lg shadow-2xl mb-6 flex flex-col items-center">
            ...
            <p className="text-gray-400 mb-1 text-center">
              Opening Time:{" "}
              {canteenData?.openingTime
                ? formatTime(canteenData.openingTime)
                : "NA"}
            </p>
            <p className="text-gray-400 mb-1 text-center">
              Closing Time:{" "}
              {canteenData?.closingTime
                ? formatTime(canteenData.closingTime)
                : "NA"}
            </p>
            <p className="text-gray-400 mb-1 text-center">
          ...
          <div className="text-center">
            ...
            <form onSubmit={handleSearchSubmit}>
              ...
              <button
                type="submit"
                className="sm:ml-2 mb-4 py-2 px-4 rounded-lg bg-[#76ABAE] text-white w-full sm:w-auto"
              >
                Search
              </button>
            </form>
          ...
          <div className="grid grid-cols-1 gap-4 mb-24 sm:gap-6 justify-items-center">
            {itemsToDisplay.length > 0 ? (
              itemsToDisplay.map((item) => (
                <div
                  key={item._id}
                  className="bg-[#31363F] sm:mt-0 sm:mb-0 mb-5 mt-5  rounded-lg shadow-lg w-full sm:w-7/12 flex flex-col sm:flex-row justify-between items-center relative sm:p-4 p-4 pb-8"
                >
                  ...
                  <div className="relative flex items-center">
                    ...
                    <div className="absolute top-2 right-2">
                      {favouriteItems.some(
                        (fav) => fav.item._id === item._id
                      ) ? (
                        <AiFillHeart
                          className="text-red-500 text-2xl sm:text-xl cursor-pointer"
                          onClick={(e) =>
                    ...
                    <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2 w-full flex justify-center">
                      {cartItemMap.has(item._id) ? (
                        <div className="flex items-center justify-center space-x-0">
                          ...
                          <span className="px-4 py-1 bg-[#31363F] text-white">
                            {cartItemMap.get(item._id)}
                          </span>
                          <button
              ...
              <div className="text-center text-2xl mt-32 sm:text-4xl font-bold text-white">
                No dishes found
              </div>
            )}
          </div>
